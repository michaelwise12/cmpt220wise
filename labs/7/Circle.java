...
public class Circle {
	...
	// This method returns true if the radius of a circle is greater than the distance between the 
	// two centers of the circles (by using the distance formula) plus the radius of the smaller circle
	boolean contains(Circle circle) {
		return radius > Math.sqrt(Math.pow((x - circle.getX()), 2) + Math.pow((y - circle.getY()), 2)) + circle.getRadius();
	}
	// Method that checks if circle overlaps with another circle
